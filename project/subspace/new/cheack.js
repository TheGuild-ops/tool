...
(async () => {
  try {
    const lastAddressRaw = fs.readFileSync(`/root/node/subspace/keyLast.json`);
    ...
    lastAddress.forEach(async (element) => {
      ...
      try {
        fs.writeFileSync('/root/node/subspace/keyLast.json', dataAddress);
        // file written successfully
      } catch (err) {
        console.error(err);
      }
